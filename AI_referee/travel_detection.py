...
while cap.isOpened():
    ...
    if success:
        ...
        keypoints_obj = pose_results[0].keypoints
        if keypoints_obj is None or not hasattr(keypoints_obj, "data"):
            print("No keypoints detected, skipping frame.")
            continue

        keypoints_np = keypoints_obj.data.cpu().numpy()  # shape: (N, 17, 3)
        print("keypoints_np.shape:", keypoints_np.shape)

        if keypoints_np.shape[0] == 0:
            print("No person detected, skipping frame.")
            continue

        person_keypoints = keypoints_np[0]  # shape: (17, 3)
        print("type(person_keypoints):", type(person_keypoints))
        print("person_keypoints:", person_keypoints)
        print("person_keypoints dtype:", getattr(person_keypoints, 'dtype', 'no dtype'))

        rounded_results = np.round(person_keypoints, 1)
